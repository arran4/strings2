...
func TestLowercamel_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewLowercamel()

	called := false
	cmd.CommandAction = func(c *Lowercamel) error {
		called = true
		return nil
	}
...
func TestLowercamel_ExecuteHelpAndUnknownFlags(t *testing.T) {

...
func (c *Wordstocamel) Usage() {
	err := executeUsage(os.Stderr, "wordstocamel_usage.txt", UsageDataWordstocamel{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Wordstocamel) UsageRecursive() {
	err := executeUsage(os.Stderr, "wordstocamel_usage.txt", UsageDataWordstocamel{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Wordstocamel) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("wordstocamel failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewWordstocamel() *Wordstocamel {
	set := flag.NewFlagSet("wordstocamel", flag.ContinueOnError)
	v := &Wordstocamel{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Wordstocamel) error {
		...
		cli.WordsToCamel(c.input, c.output, c.jsonInput, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.strict, c.args...)
		return nil
	}

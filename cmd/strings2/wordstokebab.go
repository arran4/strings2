...
func (c *Wordstokebab) Usage() {
	err := executeUsage(os.Stderr, "wordstokebab_usage.txt", UsageDataWordstokebab{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Wordstokebab) UsageRecursive() {
	err := executeUsage(os.Stderr, "wordstokebab_usage.txt", UsageDataWordstokebab{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Wordstokebab) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("wordstokebab failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewWordstokebab() *Wordstokebab {
	set := flag.NewFlagSet("wordstokebab", flag.ContinueOnError)
	v := &Wordstokebab{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Wordstokebab) error {
		...
		cli.WordsToKebab(c.input, c.output, c.jsonInput, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.strict, c.args...)
		return nil
	}

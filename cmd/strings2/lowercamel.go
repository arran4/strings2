...
type Lowercamel struct {
	*RootCmd
	...
	CommandAction   func(c *Lowercamel) error
}

type UsageDataLowercamel struct {
	*Lowercamel
	Recursive bool
}

func (c *Lowercamel) Usage() {
	err := executeUsage(os.Stderr, "lowercamel_usage.txt", UsageDataLowercamel{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Lowercamel) UsageRecursive() {
	err := executeUsage(os.Stderr, "lowercamel_usage.txt", UsageDataLowercamel{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Lowercamel) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("lowercamel failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewLowercamel() *Lowercamel {
	set := flag.NewFlagSet("lowercamel", flag.ContinueOnError)
	v := &Lowercamel{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Lowercamel) error {
		...
		cli.LowerCamel(c.input, c.output, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.mixCaseSupport, c.noSmartAcronyms, c.numberSplitting, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.acronym, c.acronymFromFile, c.strict, c.args...)
		return nil
	}

...
var _ Cmd = (*Screamingdelimited)(nil)

type Screamingdelimited struct {
	*RootCmd
	Flags           *flag.FlagSet
	input           string
	output          string
	...
	CommandAction   func(c *Screamingdelimited) error
}

type UsageDataScreamingdelimited struct {
	*Screamingdelimited
	Recursive bool
}

func (c *Screamingdelimited) Usage() {
	err := executeUsage(os.Stderr, "screamingdelimited_usage.txt", UsageDataScreamingdelimited{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Screamingdelimited) UsageRecursive() {
	err := executeUsage(os.Stderr, "screamingdelimited_usage.txt", UsageDataScreamingdelimited{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Screamingdelimited) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("screamingdelimited failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewScreamingdelimited() *Screamingdelimited {
	set := flag.NewFlagSet("screamingdelimited", flag.ContinueOnError)
	v := &Screamingdelimited{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Screamingdelimited) error {
		...
		cli.ScreamingDelimited(c.input, c.output, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.mixCaseSupport, c.noSmartAcronyms, c.numberSplitting, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.acronym, c.acronymFromFile, c.strict, c.args...)
		return nil
	}

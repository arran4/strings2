...
var _ Cmd = (*Screamingkebab)(nil)

type Screamingkebab struct {
	*RootCmd
	Flags           *flag.FlagSet
	input           string
	output          string
	...
	CommandAction   func(c *Screamingkebab) error
}

type UsageDataScreamingkebab struct {
	*Screamingkebab
	Recursive bool
}

func (c *Screamingkebab) Usage() {
	err := executeUsage(os.Stderr, "screamingkebab_usage.txt", UsageDataScreamingkebab{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Screamingkebab) UsageRecursive() {
	err := executeUsage(os.Stderr, "screamingkebab_usage.txt", UsageDataScreamingkebab{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Screamingkebab) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("screamingkebab failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewScreamingkebab() *Screamingkebab {
	set := flag.NewFlagSet("screamingkebab", flag.ContinueOnError)
	v := &Screamingkebab{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Screamingkebab) error {
		...
		cli.ScreamingKebab(c.input, c.output, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.mixCaseSupport, c.noSmartAcronyms, c.numberSplitting, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.acronym, c.acronymFromFile, c.strict, c.args...)
		return nil
	}

...
func (c *Wordstodarwin) Usage() {
	err := executeUsage(os.Stderr, "wordstodarwin_usage.txt", UsageDataWordstodarwin{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Wordstodarwin) UsageRecursive() {
	err := executeUsage(os.Stderr, "wordstodarwin_usage.txt", UsageDataWordstodarwin{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Wordstodarwin) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("wordstodarwin failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewWordstodarwin() *Wordstodarwin {
	set := flag.NewFlagSet("wordstodarwin", flag.ContinueOnError)
	v := &Wordstodarwin{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Wordstodarwin) error {
		...
		cli.WordsToDarwin(c.input, c.output, c.jsonInput, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.strict, c.args...)
		return nil
	}

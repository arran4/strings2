...
func TestWordstokebab_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewWordstokebab()

	called := false
	cmd.CommandAction = func(c *Wordstokebab) error {
		called = true
		return nil
	}
...
func TestWordstokebab_ExecuteHelpAndUnknownFlags(t *testing.T) {

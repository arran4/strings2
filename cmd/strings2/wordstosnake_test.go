...
func TestWordstosnake_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewWordstosnake()

	called := false
	cmd.CommandAction = func(c *Wordstosnake) error {
		called = true
		return nil
	}
...
func TestWordstosnake_ExecuteHelpAndUnknownFlags(t *testing.T) {

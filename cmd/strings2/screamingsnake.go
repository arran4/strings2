...
var _ Cmd = (*Screamingsnake)(nil)

type Screamingsnake struct {
	*RootCmd
	Flags           *flag.FlagSet
	input           string
	output          string
	...
	CommandAction   func(c *Screamingsnake) error
}

type UsageDataScreamingsnake struct {
	*Screamingsnake
	Recursive bool
}

func (c *Screamingsnake) Usage() {
	err := executeUsage(os.Stderr, "screamingsnake_usage.txt", UsageDataScreamingsnake{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Screamingsnake) UsageRecursive() {
	err := executeUsage(os.Stderr, "screamingsnake_usage.txt", UsageDataScreamingsnake{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Screamingsnake) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("screamingsnake failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewScreamingsnake() *Screamingsnake {
	set := flag.NewFlagSet("screamingsnake", flag.ContinueOnError)
	v := &Screamingsnake{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Screamingsnake) error {
		...
		cli.ScreamingSnake(c.input, c.output, c.delimiter, c.screaming, c.whispering, c.firstUpper, c.firstLower, c.mixCaseSupport, c.noSmartAcronyms, c.numberSplitting, c.nonAlphanumeric, c.delimiters, c.delimitersFunc, c.acronym, c.acronymFromFile, c.strict, c.args...)
		return nil
	}

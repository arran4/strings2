...
func TestWordstotitle_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewWordstotitle()

	called := false
	cmd.CommandAction = func(c *Wordstotitle) error {
		called = true
		return nil
	}
...
func TestWordstotitle_ExecuteHelpAndUnknownFlags(t *testing.T) {
